...
train = pd.read_csv('./extract-data/train.csv')
test = pd.read_csv('./extract-data/test.csv')
# train['trigger'] = train['trigger'].apply(lambda x:x+'_value')

lb = (train['trigger'].value_counts()>1).reset_index()
lb.columns = ['lb','flag']
...
def extract_spoes(text):
    ...
    start = np.where(subject_preds[0, :, 0] > tt)[0]
    end = np.where(subject_preds[0, :, 1] > tt)[0]
    subjects = []
    for i in start:
        j = end[end >= i]
        if len(j) > 0:
    ...
    if subjects:
        ...
        for subject, object_pred in zip(subjects, object_preds):
            start = np.where(object_pred[:, :, 0] > tt)
            end = np.where(object_pred[:, :, 1] > tt)
...
test['submit'] = test['news'].apply(extract_spoes)
test['time'] = test['news'].apply(get_strtime)
...
def recommendation_user_list(result):
    z = result.groupby(['id'])['submit'].apply(lambda x:np.concatenate(list(x))).reset_index()
    i = pd.concat([pd.Series(row['id'], row['submit']) for _, row in z.iterrows()]).reset_index()
    i.columns = ['submit','id']
    return i
...
print ('this is predict time !!!!!!!    ')
submit = recommendation_user_list(test)
submit['trigger'] = submit['submit'].apply(lambda x:x[1])
submit['object'] = submit['submit'].apply(lambda x:x[0])
submit['subject'] = submit['submit'].apply(lambda x:x[2])
submit = submit.merge(test[['id','time']],on = 'id',how='left')
submit['location'] = np.nan
del submit['submit']
example = pd.read_csv('./extract-data/example.csv')[['id']]
example = example.drop_duplicates()
submit = example.merge(submit,on='id',how = 'left')

# In[19]:

submit.to_csv('submit_kaggle_1442.csv',index= False)
